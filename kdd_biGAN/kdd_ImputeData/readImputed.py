...
class ReadImputedKddData:
    ...
    def ready(self,y):
        this_y=[]
        print("Reading labels...")  # 添加调试信息
        for line in y.readlines():
            d=[]
            words=line.strip().split(",")
            print("Original label:", line.strip())  # 打印原始行
            for w in words:
                if w=='':
                    continue
                try:
                   # 确保转换为浮点数后再转为整数
                    val = int(float(w))
                    print("Converting label: %s -> %d" % (w, val))  # 打印转换过程
                    d.append(val)
                except ValueError as e:
                    print("Error converting label %s: %s" % (w, e))
                    d.append(0)  # 或者根据需求设置默认值
            this_y.append(d)
            print("Complete label:", d)
        return this_y
...
if __name__ == '__main__'     :
    ...
    #dt=ReadImputedKddData("../Gan_Imputation/imputation_test_results/WGAN_no_mask/30_8_16_64_0.001_400_True_True_True_0.1_0.5/")  
    dt=ReadImputedKddData("../Gan_Imputation/imputation_test_results/WGAN_no_mask/30_8_128_64_0.001_400_True_True_True_0.1_0.5/")  
    dt.load()
