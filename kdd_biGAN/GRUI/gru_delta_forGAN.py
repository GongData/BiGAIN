...
class grui(object):
    ...
    def train(self):
        ...
        while epochcount<self.epoch:
            ...
            for data_x,data_y,data_mean,data_m,data_delta,data_x_lengths,data_lastvalues,_,_,_ in dataset.nextBatch():
                ...
                print("data_y shape:", data_y.shape) # to check if the data is correct
                print("n_classes:", self.n_classes) # to check if the data is correct
                
                # 在feed数据之前转换标签为one-hot
                labels = data_y
                one_hot_labels = np.zeros((labels.shape[0], self.n_classes))
                for i, label in enumerate(labels):
                    one_hot_labels[i][int(label)] = 1
                
                # 使用转换后的one_hot_labels
    ...
    def test(self,dataset, epoch):
        #print("Test Data Type:", type(dataset))
        #print("- Data Size (dataset.x):", len(dataset.x) if hasattr(dataset, 'x') else "no")
        #print("- Batch size:", self.batch_size)
        #print("- Expected Batch Number:", len(dataset.x) // self.batch_size if hasattr(dataset, 'x') else "no")
        ...
        for data_x,data_y,data_mean,data_m,data_delta,data_x_lengths,data_lastvalues,_,_,_ in dataset.nextBatch():
            ...
            summary_str,acc,pred = self.sess.run([self.sum, self.accuracy,self.pred], feed_dict={\
                self.x: data_x,\
                self.y: one_hot_labels,  # 使用转换后的 one_hot_labels
                self.m: data_m,\
                self.delta: data_delta,\
                self.mean: data_mean,\
                self.x_lengths: data_x_lengths,\
            ...
            # 在计算 AUC 时使用原始标签
            try:
                auc = metrics.roc_auc_score(labels, pred[:, 1])  # 使用正类的预测概率
                
                # 修改后：确保标签和预测值的格式正确
                labels_flat = labels.ravel()  # 将标签展平为一维数组
                pred_probs = pred[:, 1]  # 获取正类的预测概率
                
                # 打印调试信息
                print("Debug - Labels:", labels_flat)
                print("Debug - Predictions:", pred_probs)
                
                # 确保标签中包含两个类别
                if len(np.unique(labels_flat)) < 2:
                    print("Warning: Only one class present in batch, skipping AUC calculation")
                    continue
                
                auc = metrics.roc_auc_score(labels_flat, pred_probs)
                totalauc += auc
                auccounter += 1
                print("Batch: %4d time: %4.4f, acc: %.8f, auc: %.8f" \
                          % (counter, time.time() - start_time, acc, auc))
            except ValueError as e:
                print("AUC calculation error:", str(e))  # 打印具体的错误信息
