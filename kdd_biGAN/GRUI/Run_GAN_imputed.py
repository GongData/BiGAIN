...
from kdd_ImputeData import readImputed
import gru_delta_forGAN 
import numpy as np  # 添加这一行
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='manual to this script')
    parser.add_argument('--gpus', type=str, default = None)
    parser.add_argument('--batch-size', type=int, default=128) #原先128
    ...
    parser.add_argument('--n-inputs', type=int, default=10) # 原先41 改成KDD DATASET 10
    ...
    logdir=args.log_dir
    base=args.data_path
    #data_paths=["30_8_128_64_0.001_400_True_True_True_0.15_0.5"] #for physionet
    data_paths=["30_8_128_64_0.01_400_True_True_True_0.15_0.5"] # for kdd
    max_auc = 0.0
    for d in data_paths:
        ...
        dt_train=readImputed.ReadImputedKddData(args.data_path)
        dt_train.load()
        #PRINT TEST
        print("TRAIN DATASET DISTRIBUTION:")
        print(np.unique(dt_train.y, return_counts=True))

        dt_test=readImputed.ReadImputedKddData(args.data_path.replace("imputation_train_results","imputation_test_results"))
        dt_test.load()
          
        print("TEST DATASET DISTRIBUTION:")
        print(np.unique(dt_test.y, return_counts=True))
