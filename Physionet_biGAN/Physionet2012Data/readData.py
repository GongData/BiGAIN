...
class ReadPhysionetData():
    ...
    def __init__(self, dataPath, labelPath,isNormal,isSlicing,sliceGap=60):
        ...
        for fileName in fileNames:
            ...
            # 初始化前兩個時間戳記
            #lastTime=0
            lastTime1 = [-1.0] * (len(dic)-1)
            lastTime2 = [-1.0] * (len(dic) - 1)
        ...
        for h in range(len(self.x)):
            ...
            for i in range(len(oneFile)):
                ...
                for j in range(len(oneFile[i])):
                    ...
                    if one_m[i-1][j]==1 and i > 1: 
                        # Apply new time calculation: current time + (previous time * time before last * 0.005)
                        one_deltaPre[i][j] = one_time[i] - (one_time[i-1] * one_time[i-2] * 0.5)
                    elif one_m[i-1][j]==1: #當沒有t-2 的情況下
                        one_deltaPre[i][j]=one_time[i] - one_time[i-1]  # Fallback if not enough history

                    if one_m[i][j]==1: #當前有資料
                        one_lastvalues[i][j]=oneFile[i][j]
                    if one_m[i][j]==0: #如果當前沒有資料, 則用前一個時間點資料補充
                        one_lastvalues[i][j]=one_lastvalues[i-1][j]
                    '''
